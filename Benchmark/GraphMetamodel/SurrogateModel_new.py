...
class SurrogateInputModel:
    ...
    def __init__(self, name, state, initial, fx, dt, input_dt, total_time,  
                transition_cov_scale, emission_cov_scale, noise_model_type, unit):   

        self.modelname = name
        self.state = state
        self.n_state = len(self.state)
        self.initial = initial
        self.initial_noise = 0.01*abs(np.diag(initial))
        self.fx = fx
        self.dt = dt
        ...
        for k in range(n_repeat): # repeat for different observations
            ...
            for t in range(self.n_step):
                ...
                if filepath is not None:
                    ...
                    # print(*mean_ts, file=output, sep=',')
                    # for i in range(2):
                    #     print(*surrogate_ts.P[i], file=output, sep=',')
